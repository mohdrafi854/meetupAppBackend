...
app.get("/events/", async (req, res) => {
  ...
  try {
    ...
    if (event) {
      const events = await Events.find({ event });
...
async function readEventsByUpdateById(eventId, updateToData) {
  try {
    const events = await Events.findByIdAndUpdate(eventId, updateToData, {
      new: true,
    });
    return events;
  } catch (error) {
...
app.post("/events/:eventId", async (req, res) => {
  try {
    const updateEvents = await readEventsByUpdateById(
      req.params.eventId,
      req.body
    );
    if (updateEvents) {
...
app.patch("/events/:id/edit", async (req, res) => {
  try {
    const { id } = req.params;

    if (!id) {
      res.status(400).json({ error: "Event ID does not exist" });
    }
